...
- **Real code:** `src/config/`: `LocalConfig` (machine resources; `DEVICE` is CUDA → Apple MPS → CPU via `detect_device()`), `DQNConfig` and `PPOConfig` (dataclasses whose fields are Stable-Baselines3 constructor arguments: `DQN(policy, env, **asdict(DQNConfig()))`; put algorithm hyperparameters here, not in `BaseConfig`). The Typer CLI shell in `src/cli/main.py` (`python -m src.cli.main --help` works).
...
- **Baseline checks:** `make lint` and `make test` pass (13 tests; ~12s because one test drives real Chrome and is skipped if Chrome is unavailable). Makefile targets use `poetry run`, so no venv activation is needed. Keep both green.
