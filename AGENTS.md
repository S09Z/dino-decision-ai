...
- **Baseline checks:** `make lint` and `make test` pass (60 tests; ~15s because one test drives real Chrome and is skipped if Chrome is unavailable). Makefile targets use `poetry run`, so no venv activation is needed. Keep both green.
