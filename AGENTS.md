...
- **Baseline checks:** `make lint` and `make test` pass (33 tests; ~17s because one test drives real Chrome and is skipped if Chrome is unavailable). Makefile targets use `poetry run`, so no venv activation is needed. Keep both green.
