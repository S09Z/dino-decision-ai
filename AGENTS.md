...
make train      # python -m src.cli.main train --agent dqn (also: poetry run dino-ai train)
...
- **Real code:** `src/config/`: `LocalConfig` (machine resources; `DEVICE` is CUDA → Apple MPS → CPU via `detect_device()`), `DQNConfig` and `PPOConfig` (dataclasses whose fields are Stable-Baselines3 constructor arguments: `DQN(policy, env, **asdict(DQNConfig()))`; put algorithm hyperparameters here, not in `BaseConfig`). `src/performance/gpu_detector.py`: hardware report and warnings (`python -m src.performance.gpu_detector`); its recommended settings come from `LocalConfig`. `src/utils/logger.py`: `get_logger(__name__)` (rich console + rotating `logs/dino_ai.log`). `DQNAgent` and `PPOAgent` (`src/models/`, both `SB3Agent` subclasses; `AGENTS` in `src/models/__init__.py` maps CLI names): SB3 DQN/PPO with `CnnPolicy` on `make_dino_env()` (`src/training/envs.py`: 4 stacked frames, Monitor-wrapped); PPO pauses the game during its updates. `python -m src.training.smoke --agent dqn|ppo` trains briefly and compares with a random policy. `train` and `smoke` show a progress bar (tqdm). `src/profiling/`: `@profiler.profile` function timing, `resource_usage()` (RAM/CPU of Python + Chrome, GPU memory); `python -m src.profiling.baseline` writes the baseline report. `MetricsDB` (`src/monitoring/local_db.py`): SQLite metrics store (episodes, training, performance, routing). `src/models_mgmt/`: `CheckpointManager` (keep best N checkpoints per agent) and `ModelRegistry` (versioned releases in `models/vX.Y.Z/`, metrics, changelog). The Typer CLI (`src/cli/main.py`, also `poetry run dino-ai`): `train --agent dqn|ppo` (episodes/training/performance to `MetricsDB`, best checkpoints via `TrainingMonitor` in `src/training/callbacks.py`), `eval` (best checkpoint; helpers in `src/evaluation/evaluate.py`), `profile`.
...
- The game is real time and keeps running while the agent is not stepping. Anything slow between steps (like a PPO update) must pause it (`env.pause()`/`resume()`), or the dino crashes with nobody playing.
